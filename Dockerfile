...
RUN mkdir /tmp/codeception /tmp/codeception/_log /tmp/codeception/_data && chmod 777 -R /tmp/codeception
...
ENV FIREFOX_VERSION="${FF_VER}" \
  ...
  SELENIUM_NODE_PARAMS="" \
  SELENIUM_NODE_PROXY_PARAMS="" \
  CHROME_ARGS="--no-sandbox --disable-setuid-sandbox --disable-gpu --disable-infobars --start-maximized" \
